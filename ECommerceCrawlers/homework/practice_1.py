...
print(str(lines_total_from_2012_11) + '个2012年11月发布的tweets ')
...
print(str(lines_by_created) + '天的数据 ')
...
for k, v in dateline_by_user.items():
    us = v.items()
    sorted(us,key=lambda k: k[1], reverse=True)
    dateline_by_user[k] = {list(us)[0][0]: list(us)[0][1]}
print('每一天发表tweets最多的用户 ' + str(dateline_by_user))
...
print('2012-11-03 每个小时的发布tweets的频率' + str(hour_timeline_from_2012_11_03))
...
print('该文本里，来源的相关信息和次数' + str(source_list) + ' 个')
...
for line in lines:
    if line[keys['rt_v_url']].startswith('https://twitter.com/umiushi_no_uta'):
        umi_total += 1
print('转发URL中：以："https://twitter.com/umiushi_no_uta"开头的有' + str(umi_total) )
...
print('uid:573638104 发了'+ str(tweets_total_from_573638104) + '个微博')
...
# todo 如果有多个最多怎么办？
print('uid:'+ user_by_max_content + ' 发的微博内容长度最长')
...
print('uid:'+ user_by_max_rt + ' 转发的URL最多')
...
# todo 如果有多个最多怎么办？
print('uid:'+ uid_by_max_tweets_on_hour11 + ' 11点钟，谁发的微博次数最多')
...
# todo 如果有多个最多怎么办？
print('uid:'+ uid_by_max_v_url + ' 用户的源微博URL次数最多')
assert type(uid_by_max_v_url) == str

print
'运算时间：%s' % (time.time() - now)  # 整体运行时间
