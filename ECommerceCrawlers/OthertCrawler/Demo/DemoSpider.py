    ...
    def parse_content(self,content,VIEWSTATE="",EVENTVALIDATION="",flag=True):
        ...
        if len(tr_list):
            for tr in tr_list:
                ...
                if normalPrice and normalPrice.startswith("￥"):
                    normalPrice = normalPrice.split("￥")[1]
                insert_sql = """
                insert into anpel_info(id,name,cas,brand,specificationsAndModels,normalPrice,InventoryAndLeadTime) values (%s,%s,%s,%s,%s,%s,%s)
                """
                self.cursor.execute(insert_sql, (item['id'],item['name'],item['CAS'],item['brand'],item['specificationsAndModels'],normalPrice,item['InventoryAndLeadTime']))
                self.conn.commit()
        if flag:
