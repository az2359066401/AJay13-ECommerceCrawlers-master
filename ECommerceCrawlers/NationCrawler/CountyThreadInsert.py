...
class ThreadInsert(object):
    ...
    def getData(self):
        st = time.time()
        with open("county.txt", "rb") as f:
            data = []
            for line in f:
                line = re.sub("\s", "", str(line, encoding="utf-8"))
                line = tuple(line[1:-1].split("\"\""))
                line = tuple((line[0],line[1],int(line[2]),line[3]))
                print(line)
                data.append(line)
    ...
    def mysql_insert(self, *args):
        con = self.pool.connection()
        cur = con.cursor()
        sql = "INSERT INTO county(name, code, type, parent_code) VALUES(%s, %s, %s, %s)"
        try:
